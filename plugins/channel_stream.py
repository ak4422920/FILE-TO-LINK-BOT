...
async def channel_receive_handler(bot: Client, broadcast: Message):
    try:
        ...
        raw_stream = f"{URL}watch/{msg.id}/ak.mkv?hash={get_hash(msg)}"
        ...
        buttons_list = [
            [InlineKeyboardButton(" 🎬 Stream Now ", url=stream),
             InlineKeyboardButton(" ⬇️ Download ", url=download)],
            [InlineKeyboardButton(' 📁 Click here to Get File ', url=file_link)]
        ]
        if IS_SHORTLINK:
            buttons_list.append([
